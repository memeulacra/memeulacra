...
import { AgentRequest, AgentResponse } from '@/app/api/agent/route'
 ...
 */
async function messageAgent(userMessage: string, memeId: string): Promise<string | null> {
  try {
    const response = await fetch('/api/agent', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userMessage, memeId } as AgentRequest),
    })
...
export function useAgent() {
   ...
   */
  const sendMessage = async (input: string, memeId: string) => {
    if (!input.trim()) return

    setMessages(prev => [...prev, { text: input, sender: 'user' }])
    setIsThinking(true)

    const responseMessage = await messageAgent(input, memeId)
